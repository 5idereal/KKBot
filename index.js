...
const projectId = 'line-bot-294201';
...
async function handleEvent(event) {
  ...
  switch (responses[0].queryResult.intent['displayName']) {
    ...
    case "Test": featuredplaylist(event.replyToken); break;
  }
}
...
function featured(token) {
  axios.get(encodeURI('https://api.kkbox.com/v1.1/featured-playlists?territory=TW&limit=15'), { headers: { 'authorization': 'Bearer ' + kkbox_token } }).then(response => {
    let a = [];
...
function featuredplaylist(token) {
  axios.get(encodeURI('https://api.kkbox.com/v1.1/featured-playlist-categories?territory=TW&limits=10'), { headers: { 'authorization': 'Bearer ' + kkbox_token } }).then(response => {
    client.replyMessage(token, test)
    console.log(response.data.data);
  });
}
